				...
				<ul >
					<li >	
						...
						<select id="s_use_yn" name="s_use_yn" class="SYSelectBox01" onchange="sch_list('schForm');">
							<option value="ALL" {? _REQUEST.s_use_yn == 'ALL'} selected{/}>==선택==</option>
							<option value="Y" {? _REQUEST.s_use_yn == 'Y' ||  _REQUEST.s_use_yn == ''} selected{/}>Y</option>
							<option value="N" {? _REQUEST.s_use_yn == 'N'} selected{/}>N</option>
						</select>
			...
			<ul>
				...
				<li class="Right">
					...
					<select id="s_orderby" name="s_orderby" class="SYSelectBox01" onchange="sch_list('schForm');">
						...
						<option value="3" {? _REQUEST.s_orderby == '3'} selected{/}>호텔명 오름차순</option>
						<option value="4" {? _REQUEST.s_orderby == '4'} selected{/}>호텔명 내림차순</option>
					</select>
		...
		<table class="SYManagerTable01">
			...
			{:}
				...
				<tr >						
					...
					<td  class="ce">{.hotel_code}</td>
					<td  class="ce">{.nat_cd}</td>
					<td  class="ce">{.airport_code}</td>
					<td  class="ce">{.hotel_nm}</td>
					<td  class="ce">{.hotel_nm_en}</td>
					<td  class="ce">{.hotel_nm_jp}</td>
					<td  class="ce">{.grade}</td>
					

					<td  class="ce">{.view_yn_txt}</td>
					<td  class="ce">
						<input type="button" class="SYButtonType03" value="수정" onclick="code_hotel_write('{.seq}')" >
						
					</td>
					<td  class="ce">
						<input type="button" class="SYButtonDelete01" onclick="code_hotel_del('{.seq}')" >
					</td>
					<td > </td>
				</tr>
				{/}

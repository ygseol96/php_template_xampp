...
<script>
    ...
    // 대상자 추가
    $("button.btn-primary-soft").click(function() {
        var member = $("input[type='text']").val();
        var html = '<div class="flex items-center gap-2 py-1.5 px-3 border border-slate-300 bg-white rounded-full">';
        html += '<div>'+member+'</div>';
        html += '<button class="btn p-1 btn-outline-danger rounded-full"><i data-lucide="x" class="w-4 h-4"></i></button>';
        html += '</div>';
        $("div.overflow-y-auto").append(html);
    });

...
<div class="intro-y box p-5 mt-4">
    <div class="flex flex-col md:flex-row gap-3 mb-3">
        ...
        <div class="flex-1 flex flex-wrap items-center gap-2">
            <input type="text" class="form-control w-full md:w-40" value="{data['data']['name_kr']}" readonly>
            <input type="text" class="form-control w-full md:w-40" value="{data['data']['hp_number']}" readonly>
            <input type="text" class="form-control w-full md:w-40" value="{data['data']['account']}" readonly>
        </div>
    </div>
    <div class="flex flex-col md:flex-row gap-3 mb-3">
        <div class="w-full md:w-40 pt-2 font-semibold">문의구분</div>
        <div class="flex-1">
            <input type="text" class="form-control w-full md:w-52" value="{data['data']['gubun_name']}" readonly>
        </div>
    </div>
    <div class="flex flex-col md:flex-row gap-3 mb-3">
        <div class="w-full md:w-40 pt-2 font-semibold">문의제목</div>
        <div class="flex-1">
            <input type="text" class="form-control" value="{data['data']['subject']}" readonly>
        </div>
    </div>
    <div class="flex flex-col md:flex-row gap-3 mb-3">
        <div class="w-full md:w-40 pt-2 font-semibold">문의내용</div>
        <div class="flex-1">
            <textarea class="resize-none form-control h-40" readonly>{data['data']['content']}</textarea>
{*            <input type="text" class="form-control" value="회원가입 문의입니다." readonly>*}
    ...
    <div class="flex flex-col md:flex-row gap-3 mb-3">
        ...
        <div class="flex-1">
            <!--{@data['data']['inquiry_files']}-->
                <button class="btn btn-outline-secondary" onclick="window.open('{ .up_name }', '_blank');">{ .ori_name + '.' + .file_ext }</button>
            <!--{/}-->
        </div>
    </div>
</div>
...
<div class="intro-y box p-5 mt-4">
    <div class="flex flex-wrap flex-col md:flex-row gap-3 mb-3">
        <div class="flex flex-col md:flex-row items-center gap-3">
            ...
            <input type="text" class="form-control w-full md:w-40" value="{data['data']['status_name']}" disabled>
        </div>
        <div class="flex flex-col md:flex-row items-center gap-3">
            <div class="w-full md:w-32 pt-1 font-semibold">답변일시</div>
            <input type="text" class="form-control w-full md:w-40" value="{data['data']['answer_date']}" disabled>
        </div>
        <div class="flex flex-col md:flex-row items-center gap-3">
            <div class="w-full md:w-32 pt-1 font-semibold">답변자</div>
            <input type="text" class="form-control w-full md:w-40" id="adminName" value="{data['data']['answer']}" disabled>
        </div>
    </div>
    <div class="flex flex-col md:flex-row gap-3 mb-3">
        <div class="w-full md:w-40 pt-2 font-semibold">답변내용</div>
        <div class="flex-1">
            <textarea id="inquiryResponseContent" class="resize-none form-control h-40">{data['data']['a_content']}</textarea>
        </div>
    </div>
    <div class="flex flex-col md:flex-row gap-3 mb-3">
        <div class="w-full md:w-40 pt-2 font-semibold">첨부파일</div>

<?php /* Template_ 2.2.8 2024/04/19 11:47:47 C:\xampp\heytee_dev\admin\_global\skin_ko\index.tpl 000020557 */ ?>
<?php $this->print_("head",$TPL_SCP,1);?>

<style type="text/css">
	.main_h1 label{font-size:12px; font-weight:normal;}
</style>
...
<link rel="stylesheet" href="/_global/skin_ko/js/jquery-jvectormap-2.0.5.css" type="text/css" media="screen"/>
<script src="/_global/skin_ko/js/jquery-jvectormap-2.0.5.min.js"></script>
<script src="/_global/skin_ko/js/jquery-jvectormap-world-mill.js"></script>
...
<form name="form1" method="post" action="">
	...
	<div id="SYRightArea">
		<div id="SYContentTitle" >
			<ul>
			   <li id="SYMenuTitle" class="Title" style="border-left:none"></li>
		...
		<h1 class="main_h1"> 목표현황 
			<label><input type="radio" name="grp" value="tot" onclick="set_main_graph('tot')" checked> 전체</label>
			<label><input type="radio" name="grp" value="fit" onclick="set_main_graph('fit')" > FIT</label>
			<label><input type="radio" name="grp" value="pass" onclick="set_main_graph('pass')" > PASS</label>
			<label><input type="radio" name="grp" value="group" onclick="set_main_graph('group')" > 그룹</label>
		</h1>
		
		<div style="width:1500px;  margin-top:10px;" >
			
		</div>
		<div id="chartArea" style="width:1500px; height:400px; margin-top:10px;border:1px solid gray;" >
			...
			function set_main_graph(arg) {
				...
				function(json) { 
					...
					if(obj.result == 'Y') {
						...
						$('#chartArea').html('');
						$('#chartArea').append('<canvas id="plan_a"style="width:1500px; height:400px;" ></canvas>');
						...
						var plan_tot = temp[9].split('||');
						var price_tot = temp[10].split('||');
						//console.log(price_grp);


						if(arg =='tot') {

							var plan_title = '전체목표 (KRW: 만원단위)';
							var plan_price = plan_tot;
							var cpl_title = '전체실적 (KRW: 만원단위)';
							var cpl_price = price_tot;
						}

						else if(arg =='group') {

							var plan_title = '그룹목표 (KRW: 만원단위)';
							var plan_price = plan_grp;
							var cpl_title = '그룹실적 (KRW: 만원단위)';
							var cpl_price = price_grp;
						}
						else if(arg =='fit') {

							var plan_title = 'FIT목표 (KRW: 만원단위)';
							var plan_price = plan_fit;
							var cpl_title = 'FIT실적 (KRW: 만원단위)';
							var cpl_price = price_fit;
						}
						else if(arg =='pass') {

							var plan_title = 'PASS목표 (KRW: 만원단위)';
							var plan_price = plan_pass;
							var cpl_title = 'PASS실적 (KRW: 만원단위)';
							var cpl_price = price_pass;
						}
						

						//console.log('plan_title = '+ plan_title);
						//console.log('plan_price = '+ plan_price);
						//console.log('cpl_title = '+ cpl_title);
						//console.log('cpl_price = '+ cpl_price);
		...
		<div id="SYBoardBox" class="SYManagerTable" style="margin-top:10px">
			<table class="SYManagerTable01" style="width:100%; float:left">
				...
				<tbody>
					...
					<tr >						
						<td  class="ce"><?php echo $TPL_VAR["area_nm"]?></td>
						<td  class="ce"> <?php echo $TPL_VAR["new_tot_cnt"]?> (<?php echo $TPL_VAR["new_a_cnt"]?> / <?php echo $TPL_VAR["new_f_cnt"]?>)</td>
						<td  class="ce"> <?php echo $TPL_VAR["ready_tot_cnt"]?> (<?php echo $TPL_VAR["ready_a_cnt"]?> / <?php echo $TPL_VAR["ready_f_cnt"]?>)</td>
						<td  class="ce"> <?php echo $TPL_VAR["mem_tot_cnt"]?> (<?php echo $TPL_VAR["mem_a_cnt"]?> / <?php echo $TPL_VAR["mem_f_cnt"]?>)</td>
						<td  class="ce"><?php echo $TPL_VAR["re_cnt"]?></td>
						<td  class="ce"><?php echo $TPL_VAR["out_cnt"]?></td>
					</tr>
					
					<tr >						
						<td  class="ce" style="background-color:#eee; font-weight:bold; height:26px" >전체</td>
						<td  class="ce" style="background-color:#eee; font-weight:bold"> <?php echo ($TPL_VAR["data_total"]["new_tot_cnt"])?> (<?php echo ($TPL_VAR["data_total"]["new_a_cnt"])?> / <?php echo ($TPL_VAR["data_total"]["new_f_cnt"])?>)</td>
						<td  class="ce" style="background-color:#eee; font-weight:bold"> <?php echo ($TPL_VAR["data_total"]["ready_tot_cnt"])?> (<?php echo ($TPL_VAR["data_total"]["ready_a_cnt"])?> / <?php echo ($TPL_VAR["data_total"]["ready_f_cnt"])?>)</td>
						<td  class="ce" style="background-color:#eee; font-weight:bold"> <?php echo ($TPL_VAR["data_total"]["mem_tot_cnt"])?> (<?php echo ($TPL_VAR["data_total"]["mem_a_cnt"])?> / <?php echo ($TPL_VAR["data_total"]["mem_f_cnt"])?>)</td>
						<td  class="ce" style="background-color:#eee; font-weight:bold"><?php echo ($TPL_VAR["data_total"]["re_cnt"])?></td>
						<td  class="ce" style="background-color:#eee; font-weight:bold"><?php echo ($TPL_VAR["data_total"]["out_cnt"])?></td>
					</tr>

<?php /* Template_ 2.2.8 2024/02/16 08:59:10 /home/agl/www/admin/_global/skin_ko/sys/code_airport_list.tpl 000007391 */ 
...
<script type="text/javascript">
	...
	function code_airport_write(seq) {
		var url = 'code_airport_write.html';
		if(seq) url += '?seq='+seq;
		window.open(url);
	}

	function code_airport_del(seq) {
		var ans = confirm('해당 코드를 삭제하시겠습니까? 삭제시 복구할 수 없습니다.');
		if(!ans) return;

		$.post('ajax_sys.php', { mode: 'code_airport_delete', seq:seq  },
			function(json) { 
...
<form name="schForm" id="schForm" >
	...
	<div id="SYRightArea" >
	   <div id="SYContentTitle">
			<ul>
			   <li id="SYMenuTitle" class="Title">공항코드</li>
			   <li id="SYMenuNavi" class="Navi">시스템관리 > 통합코드 > 공항코드 리스트</li>
			</ul>
	   </div>
				...
				<ul >
					<li >	
						...
						<select name='s_nat' id='s_nat' class="SYSelectBox01" >
								...
								<?php echo selectOption($TPL_VAR["nation"],$_REQUEST["s_nat"],'db')?>

							</select>
						<span class="bar">|</span>

						<label for="s_keyfield">공항코드, 공항명, 키워드 </label>
					    ...
					    <input type="button" class="SYButtonType04" value="초기화" onclick="location.href='code_airport_list.html'">
		...
		<div class="SYTableTitleBar">
			<ul>
				<li class="Left" style="width:600px">
					<input type="button" class="SYButtonType03" value="등록" onclick="code_airport_write()" >
					<!--
					<span class="bar">|</span>
	...
	<div id="SYBoardBox" class="SYManagerTable" style="margin-top:20px">
		<table class="SYManagerTable01">
			<tr class="Title">
				...
				<td style="width:55px;">공항코드</td>
				<td style="width:155px;">공항명 (한글)</td>
				<td style="width:155px;">공항명 (영어)</td>
				<td style="width:155px;">공항명 (일본어)</td>
				<td style="width:155px;">공항명 (중국어)</td>
...
<?php if($TPL_data_1){$TPL_I1=-1;foreach($TPL_VAR["data"] as $TPL_V1){$TPL_I1++;?>
				<tr >						
					...
					<td  class="ce"><?php echo $TPL_V1["airport_nm_kr"]?></td>
					<td  class="ce"><?php echo $TPL_V1["airport_nm_en"]?></td>
					<td  class="ce"><?php echo $TPL_V1["airport_nm_jp"]?></td>
					<td  class="ce"><?php echo $TPL_V1["airport_nm_cn"]?></td>
					<td  class="ce"><?php echo $TPL_V1["nat_cd"]?></td>
					<td  class="ce"><?php echo $TPL_V1["nat_name"]?></td>
					<td  class="ce"><?php echo $TPL_V1["city_code"]?></td>
					<td  class="ce"><?php echo $TPL_V1["city_name"]?></td>
					<td  class="ce"><?php echo dbUnEscape($TPL_V1["airport_nm_keyword"])?></td>
					<td  class="ce"><?php echo $TPL_V1["view_yn_txt"]?></td>
					<td  class="ce">
						<input type="button" class="SYButtonType03" value="수정" onclick="code_airport_write('<?php echo $TPL_V1["airp_seq"]?>')" >
						
					</td>
					<td  class="ce">
						<input type="button" class="SYButtonDelete01" onclick="code_airport_del('<?php echo $TPL_V1["airp_seq"]?>')" >
					</td>
					<td > </td>
				</tr>
<?php }}?>

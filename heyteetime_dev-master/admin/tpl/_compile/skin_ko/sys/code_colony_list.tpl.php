<?php /* Template_ 2.2.8 2024/02/16 08:59:10 /home/agl/www/admin/_global/skin_ko/sys/code_colony_list.tpl 000006182 */ 
...
<script type="text/javascript">
	...
	function code_colony_write(seq) {
		var url = 'code_colony_write.html';
		if(seq) url += '?seq='+seq;
		window.open(url);
	}

	function code_colony_del(seq) {
		var ans = confirm('해당 코드를 삭제하시겠습니까? 삭제시 복구할 수 없습니다.');
		if(!ans) return;

		$.post('ajax_sys.php', { mode: 'code_colony_delete', seq:seq  },
			function(json) { 
...
<form name="schForm" id="schForm" >
	...
	<div id="SYRightArea" >
	   <div id="SYContentTitle">
			<ul>
			   <li id="SYMenuTitle" class="Title">국가령관리</li>
			   <li id="SYMenuNavi" class="Navi">시스템관리 > 통합코드 > 국가령관리 리스트</li>
			</ul>
	   </div>
			...
			<div style="position:relative;padding:10px; border:1px solid #ddd;"  >
				<ul >
					<li >	
						...
						<label for="s_keyfield">국가령 </label>
					    ...
					    <input type="button" class="SYButtonType04" value="초기화" onclick="location.href='code_colony_list.html'">
		...
		<div class="SYTableTitleBar">
			<ul>
				<li class="Left" style="width:600px">
					<input type="button" class="SYButtonType03" value="등록" onclick="code_colony_write()" >
					<!--
					<span class="bar">|</span>
	...
	<div id="SYBoardBox" class="SYManagerTable" style="margin-top:20px">
		<table class="SYManagerTable01">
			<tr class="Title">
				...
				<td style="width:125px;">국가령</td>
				
				<td style="width:415px;">소속국가</td>
...
<?php if($TPL_data_1){$TPL_I1=-1;foreach($TPL_VAR["data"] as $TPL_V1){$TPL_I1++;?>
				<tr >						
					...
					<td  class="ce"><?php echo $TPL_V1["col_name"]?></td>
					<td  class="le"><?php echo $TPL_V1["nat_list"]?></td>

					
					<td  class="ce"><?php echo $TPL_V1["view_yn_txt"]?></td>
					<td  class="ce">
						<input type="button" class="SYButtonType03" value="수정" onclick="code_colony_write('<?php echo $TPL_V1["col_seq"]?>')" >
						
					</td>
					<td  class="ce">
						<input type="button" class="SYButtonDelete01" onclick="code_colony_del('<?php echo $TPL_V1["col_seq"]?>')" >
					</td>
					<td > </td>
				</tr>
<?php }}?>

<?php /* Template_ 2.2.8 2024/02/22 15:41:02 /home/agl/www/admin/_global/skin_ko/sys/code_hotel.tpl 000006981 */ 
...
<script type="text/javascript">
	...
	function code_hotel_write(seq) {
		var url = 'code_hotel_write.html';
		if(seq) url += '?seq='+seq;
		window.open(url);
	}

	function code_hotel_del(seq) {
		var ans = confirm('해당 코드를 삭제하시겠습니까? 삭제시 복구할 수 없습니다.');
		if(!ans) return;

		$.post('ajax_sys.php', { mode: 'code_hotel_delete', seq:seq  },
			function(json) { 
...
<form name="schForm" id="schForm" >
	...
	<div id="SYRightArea" >
	   <div id="SYContentTitle">
			<ul>
			   <li id="SYMenuTitle" class="Title">호텔관리</li>
			   <li id="SYMenuNavi" class="Navi">시스템관리 > 통합코드 > 호텔관리 리스트</li>
			</ul>
	   </div>
			...
			<div style="position:relative;padding:10px; border:1px solid #ddd;"  >
				<ul >
					<li >	
						...
						<span class="bar">|</span>
						사용여부
						<select id="s_use_yn" name="s_use_yn" class="SYSelectBox01" onchange="sch_list('schForm');">
							<option value="ALL" <?php if($_REQUEST["s_use_yn"]=='ALL'){?> selected<?php }?>>==선택==</option>
							<option value="Y" <?php if($_REQUEST["s_use_yn"]=='Y'||$_REQUEST["s_use_yn"]==''){?> selected<?php }?>>Y</option>
							<option value="N" <?php if($_REQUEST["s_use_yn"]=='N'){?> selected<?php }?>>N</option>
						</select>

					    <input type="button" class="SYButtonSearch02" value="검색" onclick="sch_list('schForm');">
						<span class="bar">|</span>

					    <input type="button" class="SYButtonType04" value="초기화" onclick="location.href='code_hotel.html'">
		...
		<div class="SYTableTitleBar">
			<ul>
				<li class="Left" style="width:600px">
					<input type="button" class="SYButtonType03" value="등록" onclick="code_hotel_write()" >
					<!--
					<span class="bar">|</span>
					<input type="button" class="SYButtonExcel03" title="엑셀다운로드" onclick="excel_down('code_hotel')">
					-->
				...
				<li class="Right">
					...
					<select id="s_orderby" name="s_orderby" class="SYSelectBox01" onchange="sch_list('schForm');">
						...
						<option value="3" <?php if($_REQUEST["s_orderby"]=='3'){?> selected<?php }?>>호텔명 오름차순</option>
						<option value="4" <?php if($_REQUEST["s_orderby"]=='4'){?> selected<?php }?>>호텔명 내림차순</option>
					</select>
	...
	<div id="SYBoardBox" class="SYManagerTable" style="margin-top:20px">
		<table class="SYManagerTable01">
			<tr class="Title">
				...
				<td style="width:65px;">호텔코드</td>
				<td style="width:65px;">국가코드</td>
				<td style="width:65px;">공항코드</td>
				<td >호텔명 (한글)</td>
				<td style="width:175px;">호텔명 (영어)</td>
				<td style="width:175px;">호텔명 (일본어)</td>
...
<?php if($TPL_data_1){$TPL_I1=-1;foreach($TPL_VAR["data"] as $TPL_V1){$TPL_I1++;?>
				<tr >						
					...
					<td  class="ce"><?php echo $TPL_V1["hotel_code"]?></td>
					<td  class="ce"><?php echo $TPL_V1["nat_cd"]?></td>
					<td  class="ce"><?php echo $TPL_V1["airport_code"]?></td>
					...
					<td  class="ce">
						<input type="button" class="SYButtonType03" value="수정" onclick="code_hotel_write('<?php echo $TPL_V1["seq"]?>')" >
						
					</td>
					<td  class="ce">
						<input type="button" class="SYButtonDelete01" onclick="code_hotel_del('<?php echo $TPL_V1["seq"]?>')" >
					</td>
					<td > </td>
				</tr>
<?php }}?>

<?php /* Template_ 2.2.8 2024/02/16 08:59:10 /home/agl/www/admin/_global/skin_ko/sys/adm_write.tpl 000009665 */ ?>
<?php $this->print_("head",$TPL_SCP,1);?>
	...
	<script type="text/javascript">
		...
		$(function() {
<?php if($TPL_VAR["editmode"]=="admin_member_update"){?>
			get_user_auth(<?php echo $_REQUEST["seq"]?>);
<?php }?>
		});
...
<?php if($TPL_VAR["editmode"]=="admin_member_insert"){?>
	...
	<form name="iform" id="iform">
		...
		<div id="SYRightArea" >
			<div id="SYContentTitle">
				<ul>
				   <li id="SYMenuTitle" class="Title"> 사용자 상세 <?php if($TPL_VAR["editmode"]=="admin_member_update"){?> - <?php echo $TPL_VAR["data"]["adm_name"]?><?php }?></li>
			...
			<table class="SYWirteTable" style="margin-top:10px">
					...
					<tr>
						...
						<td class="Content"  >
							<input type="text" name="adm_name" id="adm_name" class="SYInputStyle02"  value="<?php echo $TPL_VAR["data"]["adm_name"]?>"  autocomplete="off">
...
<?php }?>
					...
					<tr>
						...
						<td class="Content" colspan=3>
							<input type="text" name="email" id="email" class="SYInputStyle02"  value="<?php echo $TPL_VAR["data"]["email"]?>"  autocomplete="off" style="width:200px">	
					...
					<tr>
						...
						<td class="Content" >
							<select name='adm_part_seq' id='adm_part_seq' class="SYSelectBox01" onchange="get_part_auth(this.value)" >
								...
								<?php echo selectOption($TPL_VAR["part"],$TPL_VAR["data"]["adm_part_seq"],'db')?>
						...
						<td class="Content"  >
							<select name='cj_seq' id='cj_seq' class="SYSelectBox01" >
								...
								<?php echo selectOption($TPL_VAR["jk"],$TPL_VAR["data"]["cj_seq"],'db')?>
					...
					<tr>
						...
						<td class="Content"  >
							<select name='work_yn' id='work_yn' class="SYSelectBox01" >
								<option value='Y' <?php if($TPL_VAR["data"]["work_yn"]=='Y'){?>selected<?php }?>>재직중</option>
								<option value='N' <?php if($TPL_VAR["data"]["work_yn"]=='N'){?>selected<?php }?>>퇴사</option>
								<option value='W' <?php if($TPL_VAR["data"]["work_yn"]=='W'){?>selected<?php }?>>휴직중</option>
						...
						<td class="Content"  >
							<select name='confirm_yn' id='confirm_yn' class="SYSelectBox01" >
								<option value='Y' <?php if($TPL_VAR["data"]["confirm_yn"]=='Y'){?>selected<?php }?>>승인</option>
								<option value='N' <?php if($TPL_VAR["data"]["confirm_yn"]=='N'){?>selected<?php }?>>대기</option>
							</select>

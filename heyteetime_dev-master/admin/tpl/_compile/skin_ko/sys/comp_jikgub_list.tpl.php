<?php /* Template_ 2.2.8 2024/02/16 08:59:10 /home/agl/www/admin/_global/skin_ko/sys/comp_jikgub_list.tpl 000006980 */ 
...
<script type="text/javascript">
	...
	function jikgub_write(seq) {
		
		
		if(seq) {
			var submode = "comp_jikgub_update";
		}
		else {
			var submode = "comp_jikgub_insert";
		}
		...
		$.post('ajax_sys.php', { mode: 'form_comp_jikgub', submode:submode,  seq:seq},
			function(json) { 
	...
	function jk_del(seq) {
		...
		$.post('ajax_sys.php', { mode: 'comp_jikgub_delete', seq:seq},
			function(json) { 
				var obj = $.parseJSON(json); 
				if(obj.result == 'Y') {
					alert('삭제되었습니다');
	...
	function set_viewyn(seq, value) {
		//alert(seq);
		//alert(value);

		$.post('ajax_manage.php', { mode: 'mv_view', seq:seq, value:value},
			function(json) { 
				var obj = $.parseJSON(json); 
				if(obj.result == 'Y') {
					if(value == 'Y') {
...
<form name="schForm" id="schForm" >
	...
	<input type="hidden" name="s_join" id="s_join" value="<?php echo $_REQUEST["s_join"]?>">

	
	<div id="SYRightArea" >
		<div id="SYContentTitle">
			<ul>
			   <li id="SYMenuTitle" class="Title">직급 관리</li>
			   <li id="SYMenuNavi" class="Navi">시스템관리 > 권한관리 > 직급관리 리스트</li>
			</ul>
		</div>
		...
		<div class="SYTableTitleBar">
			<ul>
				<li class="Left" style="width:600px">
					<input type="button" class="SYButtonType03" value="등록" onclick="jikgub_write()" >
					<span class="bar">|</span>
...
<form name="listForm" id="listForm" >
	<input type="hidden" name="mode" id="listmode" value="comp_jikgub_order" >
...
<?php if($TPL_data_1){foreach($TPL_VAR["data"] as $TPL_V1){?>
				<tr >						
					<td  class="ce">
						<input type='hidden' name='sort1[]' value='<?php echo $TPL_V1["cj_seq"]?>' />
					...
					<td  class="ce">
						<input type="button" class="SYButtonType03" value="수정" onclick="jikgub_write('<?php echo $TPL_V1["cj_seq"]?>')" >
						
					</td>
					<td  class="ce">
						<input type="button" class="SYButtonDelete01" onclick="jk_del('<?php echo $TPL_V1["cj_seq"]?>')" >
...
<?php }}?>
	...
	<div class="btnlistarea" style="margin-top:20px">
		...
		<div class="btnlistright">
			<input type="button" class="SYButtonSubmit04" value="등록" title="등록" onclick="jikgub_write()">
			&nbsp;
		</div>
	</div>
	-->

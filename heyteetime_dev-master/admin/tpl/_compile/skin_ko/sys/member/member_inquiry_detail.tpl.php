<?php /* Template_ 2.2.8 2024/05/21 11:43:21 C:\Users\H\projects\heyteetime_dev\admin\_global\skin_ko\sys\member\member_inquiry_detail.tpl 000008519 */ ?>
...
<div class="intro-y flex flex-wrap items-center justify-between gap-2">
    <div>
        <div class="flex items-center mt-4">
            <a href="/sys/member/member_inquiry.php" class="flex items-center gap-1 text-primary">
    ...
    <div class="flex flex-wrap items-center gap-2 ml-auto">
        ...
        <button class="btn btn-danger" onclick="location.href='/sys/member/member_inquiry.php'"><i data-lucide="x" class="w-4 h-4 mr-1"></i> 등록취소</button>
    </div>
</div>
...
<div class="intro-y box p-5 mt-4">
    ...
    <div class="flex flex-col md:flex-row gap-3 mb-3">
        ...
        <div class="flex-1">
<?php if(is_array($TPL_R1=$TPL_VAR["data"]['inquiryFiles'])&&!empty($TPL_R1)){$TPL_I1=-1;foreach($TPL_R1 as $TPL_V1){$TPL_I1++;?>
                <button class="btn btn-outline-secondary"><?php echo $TPL_VAR["data"]['inquiryFiles'][$TPL_I1]->ori_name?></button>
<?php }}?>
        </div>
    </div>
</div>
...
<div class="intro-y box p-5 mt-4">
    <div class="flex flex-wrap flex-col md:flex-row gap-3 mb-3">
        <div class="flex flex-col md:flex-row items-center gap-3">
            ...
            <input type="text" class="form-control w-full md:w-40" value="<?php echo $TPL_VAR["data"]['data']['status_name']?>">
        </div>
        <div class="flex flex-col md:flex-row items-center gap-3">
            <div class="w-full md:w-32 pt-1 font-semibold">답변일시</div>
            <input type="text" class="form-control w-full md:w-40" value="<?php echo $TPL_VAR["data"]['data']['answer_date']?>">
        </div>
        <div class="flex flex-col md:flex-row items-center gap-3">
            <div class="w-full md:w-32 pt-1 font-semibold">답변자</div>
            <input type="text" class="form-control w-full md:w-40" id="adminName" value="<?php echo $TPL_VAR["data"]['data']['answer']?>">
        </div>
    </div>
    <div class="flex flex-col md:flex-row gap-3 mb-3">
    ...
    <div class="flex flex-col md:flex-row gap-3 mb-3">
        ...
        <div class="flex-1">
            ...
            <br>
            <br>
<?php if(is_array($TPL_R1=$TPL_VAR["data"]['answerFiles'])&&!empty($TPL_R1)){$TPL_I1=-1;foreach($TPL_R1 as $TPL_V1){$TPL_I1++;?>
                <button class="btn btn-outline-secondary"><?php echo $TPL_VAR["data"]['answerFiles'][$TPL_I1]->ori_name?></button>
<?php }}?>
        </div>
    </div>
</div>
...
<script>
    $('.submitInquiryResponse').on('click', function () {
        ...
        $.ajax({
            url: '/sys/member/ajax_service',
            method: 'PATCH',
            data: {
                type: 'inquiryResponse',
                idx : <?php echo $TPL_VAR["data"]['data']['idx']?>,
                answer: $('#adminName').val(),
                content: $('#inquiryResponseContent').val(),
            },
            ...
            error: function (e) {
                $('#submit_error-modal .message').text(e.responseText);
                modalOpen("submit_error-modal")
                console.log(e);
            }
        })
    ...
    $("#submit_complete-modal .btn-primary").on('click', function () {
        location.href='/sys/member/member_inquiry'
    })
</script>
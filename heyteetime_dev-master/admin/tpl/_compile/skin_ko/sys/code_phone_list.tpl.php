<?php /* Template_ 2.2.8 2024/02/16 08:59:10 /home/agl/www/admin/_global/skin_ko/sys/code_phone_list.tpl 000006551 */ 
...
<script type="text/javascript">
	...
	function code_phone_write(seq) {
		var url = 'code_phone_write.html';
		if(seq) url += '?seq='+seq;
		window.open(url);
	}

	function code_phone_del(seq) {
		var ans = confirm('해당 코드를 삭제하시겠습니까? 삭제시 복구할 수 없습니다.');
		if(!ans) return;

		$.post('ajax_sys.php', { mode: 'code_phone_delete', seq:seq  },
			function(json) { 
...
<form name="schForm" id="schForm" >
	...
	<div id="SYRightArea" >
	   <div id="SYContentTitle">
			<ul>
			   <li id="SYMenuTitle" class="Title">국가번호 코드</li>
			   <li id="SYMenuNavi" class="Navi">시스템관리 > 통합코드 > 국가번호 리스트</li>
			</ul>
	   </div>
			...
			<div style="position:relative;padding:10px; border:1px solid #ddd;"  >
				<ul >
					<li >	
						...
						<label for="s_keyfield">국가코드, 통신코드 </label>
					    ...
					    <input type="button" class="SYButtonType04" value="초기화" onclick="location.href='code_phone_list.html'">
		...
		<div class="SYTableTitleBar">
			<ul>
				<li class="Left" style="width:600px">
					<input type="button" class="SYButtonType03" value="등록" onclick="code_phone_write()" >
					<!--
					<span class="bar">|</span>
...
<?php if($TPL_data_1){$TPL_I1=-1;foreach($TPL_VAR["data"] as $TPL_V1){$TPL_I1++;?>
				<tr >						
					...
					<td  class="ce"><?php echo $TPL_V1["phone_code"]?></td>
					<td  class="ce"><?php echo $TPL_V1["view_yn_txt"]?></td>
					<td  class="ce">
						<input type="button" class="SYButtonType03" value="수정" onclick="code_phone_write('<?php echo $TPL_V1["np_seq"]?>')" >
						
					</td>
					<td  class="ce">
						<input type="button" class="SYButtonDelete01" onclick="code_phone_del('<?php echo $TPL_V1["np_seq"]?>')" >
					</td>
					<td > </td>
				</tr>
<?php }}?>

<?php /* Template_ 2.2.8 2024/06/19 15:21:22 C:\xampp\heyteetime_dev\admin\_global\skin_ko\product\prod_teetime.tpl 000011421 */ ?>
...
<?php if(is_array($TPL_R1=$TPL_VAR["data"]['f_area'])&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_V1){?>
                    <option value="<?php echo $TPL_V1["CD"]?>"><?php echo $TPL_V1["CD_NAME_KR"]?></option>
<?php }}?>
            </select>
...
<?php if(is_array($TPL_R1=$TPL_VAR["data"]['prod_kind'])&&!empty($TPL_R1)){$TPL_I1=-1;foreach($TPL_R1 as $TPL_K1=>$TPL_V1){$TPL_I1++;?>
                    <li class="flex items-center mt-0.5">
                        <input type="radio" class="form-check-input" value="<?php echo $TPL_K1?>" id="member_check_5_<?php echo $TPL_I1+ 1?>" name="filterKind">
                        <label for="member_check_5_<?php echo $TPL_I1+ 1?>" class="block w-full px-2 py-1"><?php echo $TPL_V1?></label>
                    </li>
<?php }}?>
                </ul>
            </div>
